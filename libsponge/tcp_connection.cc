...
void TCPConnection::sendSegment(bool RST){
...
void TCPConnection::segment_received(const TCPSegment &seg) {
    ...
    /* CLEAN SHUTDOWN 1 : PASSIVE CLOSE*/
    if  (
            this->_linger_after_streams_finish == false &&                  // passive close flag is set and 
            this->_sender.isFINSent() == true &&                            // FIN flag has been sent out and
            this->_sender.isNotAcknowledgedEmpty() == true                  // all the segments has been acknowledged
        )
        {
    ...
    /* CLEAN SHUTDOWN 2 : LINGER FOR A WHILE */
    else if  (   
            this->_linger_after_streams_finish == true &&                   // linger flag is set
            this->_sender.isFINSent() == true &&                            // TCPConnection has sent the FIN flag
            this->_sender.isNotAcknowledgedEmpty() == true &&               // all the segments has been acknowledged
            this->_receiver.stream_out().input_ended() == true              // FIN from peer TCP has been received    
        )
        {
...
void TCPConnection::tick(const size_t ms_since_last_tick) { 
    ...
    /* retransmit the segment immediately if segment existing */ 
    this->sendSegment(false);    
    ...
    if(this->_sender.consecutive_retransmissions() > TCPConfig::MAX_RETX_ATTEMPTS)
    {
        ...
        /* 2.1 force the sender send a empty segemnt */
    ...
    this->_LastReceivedTimer.timeElapsedBy(ms_since_last_tick);    
}
...
TCPConnection::~TCPConnection() {
    try {
        if (active()) {
            ...
            /* UNCLEAN SHUTDOWN 2 */
            /* send an empty segment from sender */
            this->_sender.send_empty_segment();
